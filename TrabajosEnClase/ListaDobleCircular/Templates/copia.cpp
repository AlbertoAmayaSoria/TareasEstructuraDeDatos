...
template <typename T>
ListaDobleCircular<T>::ListaDobleCircular() : primero(nullptr), ultimo(nullptr), tamaño(0) {
 ...
 */
template <typename T>
ListaDobleCircular<T>::ListaDobleCircular(const ListaDobleCircular& otra) : primero(nullptr), ultimo(nullptr), tamaño(0) {
...
ListaDobleCircular<T>& ListaDobleCircular<T>::operator=(const ListaDobleCircular& otra) {
    ...
    // Variables para recorrer ambas listas
    Nodo* actual = cabeza;
    ...
    // Cerramos el círculo de la lista copiando los enlaces de vuelta
 ...
 */
template <typename T>
void ListaDobleCircular<T>::insertarFinal(T valor) {
    // Creamos un nuevo nodo con el valor proporcionado.
    Nodo* nuevo = new Nodo(valor);
 ...
 */
template <typename T>
void ListaDobleCircular<T>::insertarPosicion(T valor, size_t posicion) {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::eliminarPosicion(size_t pos) {
    // Verificamos si la posición es válida.
    if (pos >= tamaño) {
        throw std::out_of_range("Posición inválida");
    }
 ...
 */
template <typename T>
void ListaDobleCircular<T>::eliminarValor(T valor) {
    // Si la lista está vacía, no hacemos nada.
    if (!primero) return;
 ...
 */
template <typename T>
void ListaDobleCircular<T>::eliminarTodasOcurrencias(T valor) {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::eliminarPorCondicion(bool (*condicion)(T)) {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::eliminarElementosRepetidos() {
 ...
 */
template <typename T>
T ListaDobleCircular<T>::obtenerPrimero() const {
 ...
 */
template <typename T>
T ListaDobleCircular<T>::obtenerUltimo() const {
    // Si la lista está vacía, lanzamos una excepción.
    if (!ultimo) {
        throw std::out_of_range("La lista está vacía.");
    }
 ...
 */
template <typename T>
T ListaDobleCircular<T>::obtenerEnPosicion(size_t posicion) const {
    // Verificamos si la posición es válida.
    if (posicion >= tamaño) {
 ...
 */
template <typename T>
int ListaDobleCircular<T>::obtenerIndice(const T& valor) const {
    // Recorremos la lista buscando el valor.
    Nodo* actual = primero;
    int indice = 0;
 ...
 */
template <typename T>
void ListaDobleCircular<T>::modificarEnPosicion(size_t posicion, T nuevoValor) {
    // Verificamos si la posición es válida.
    if (posicion >= tamaño) {
        throw std::out_of_range("Índice fuera de rango.");
    }
 ...
 */
template <typename T>
size_t ListaDobleCircular<T>::obtenerTamaño() const {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::imprimir() const {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::imprimirReversa() const {
 ...
 */
template <typename T>
T& ListaDobleCircular<T>::operator[](size_t posicion) {
 ...
 */
template <typename T>
const T& ListaDobleCircular<T>::operator[](size_t posicion) const {
    // Verificamos si el índice es válido.
    if (posicion >= tamaño) {
        throw std::out_of_range("Índice fuera de rango.");
    }
 ...
 */
template <typename T>
void ListaDobleCircular<T>::ordenarAscendente() {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::intercambiar(ListaDobleCircular<T>& otra) {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::transferirElementos(ListaDobleCircular<T>& otra, size_t inicio, size_t fin) {
 ...
 */
template <typename T>
void ListaDobleCircular<T>::transferirDesdeIndice(ListaDobleCircular<T>& otra, size_t indice) {
    // Verificamos si el índice es válido.
 ...
 */
template <typename T>
void ListaDobleCircular<T>::transferirTodos(ListaDobleCircular<T>& otra) {
